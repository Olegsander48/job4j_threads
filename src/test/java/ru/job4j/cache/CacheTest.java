...
class CacheTest {
    ...
    @Test
    public void whenMultiUpdateWithFindById() throws OptimisticException {
        var base = new Base(1, "Base", 1);
        var cache = new Cache();
        cache.add(base);
        cache.update(base);
        cache.update(cache.findById(1).get());
        assertThat(cache.findById(1).get().version()).isEqualTo(3);
    }

    @Test
    public void whenDeleteObjectWithSimilarId() throws OptimisticException {
        var base = new Base(1, "Base", 1);
        var cache = new Cache();
        cache.add(base);
        cache.delete(1);
        assertThatThrownBy(() -> cache.delete(1))
